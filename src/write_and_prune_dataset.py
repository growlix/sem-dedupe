...
def write_and_prune(
    ...
    if similarity_file.endswith('.pkl'):
        ...
        logger.info(f'Loaded {similarity_file} with {len(similarities["similarities"])} similarities')
    ...
    try:
        ...
        logger.error(f'{keep_fraction} quantile not present in {similarity_file}. Please use one of:\n')
            ...
            for sample, index in batch:
                if uid_key is not None:
                    try:
                        ...
                        logger.error(f'Could not find key {uid_key} in sample {sample}')
                        no_uid += 1
                        break
                else:
                    key = index
                ...
                if i%10000000 == 0:
                    ...
                    logger.info(f'Not found {not_found} of {i+1} samples {not_found/(i+1):.4f}')
                i += 1
    if found != 0:
        logger.info(f'\nRemoved {removed} of {found} found samples {removed/found:.4f}')
    logger.info(f'Not found {not_found} of {i+1} samples {not_found/(len(old_dataset)):.4f}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Prune and write dataset as MDS streaming dataset')
    ...
    parser.add_argument('--uid_key', type=str, default='', help='Key indicating unique id of sample. If nothing provided, will use sample index.')
