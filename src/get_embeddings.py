...
# Embedding model's max sequence length in 512, so we need a function that will a sequence into chunks of length ≤512
...
class E5Collator:
    ...
    def __call__(self, samples: List) -> Dict:
        ...
        return {'samples': samples_tokenized, 'indices': sample_indices, 'uids': uids}
...
def do_the_thing(
    ...
    **kwargs,
    ...
    if uid_key not in dataset.shards[0].column_names:
        if rank == 0:
            logging.warning(f"Key {uid_key} not found in dataset. Using index as uid.")
    else:
        use_uid = True
    ...
    for batch in dataloader:
        ...
        if use_uid:
            ind2uid.update({ind.item(): uid for ind, uid in zip(sample_indices_unique, uids)})
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Get embeddings for a streaming dataset')
    parser.add_argument('--streaming_remote', type=str)
    parser.add_argument('--streaming_local', type=str, default="/tmp/streaming_dataset")
    parser.add_argument('--save_dir', default=None, type=str) # TODO: Make this work
    parser.add_argument('--file_name', default='EMB_MEMORY.npy', type=str)
    parser.add_argument('--split', type=str, help='Dataset split (e.g. "train" or "val")')
    ...
    parser.add_argument('--batch_size_dataloader', default=320, type=int, help='Batch size for dataloader')
    parser.add_argument('--batch_size_inference', default=640, type=int, help='Batch size for inference, because batch size after tokenization and chunking will be >> batch size of samples in dataloader')
    parser.add_argument('--world_size', default=torch.cuda.device_count(), type=int)
    parser.add_argument('--post_processing_fxn', default='post_processing_bert', type=str, help='Function to post-process model outputs')
    parser.add_argument('--collator', default='e5', type=str)
    parser.add_argument('--parallel_strategy', default='mp', type=str, help="mp (multiprocessing) or dp (Data Parallel). Don't use dp.")
    parser.add_argument('--uid_key', type=str, help='unique identifier for samples. If not provided, will use sample index in dataset as uid.')
    ...
    collator = COLLATORS[args.collator.lower()](tokenizer=tokenizer, chunk_size=args.max_seq_length, instruction=args.instruction, uid_key=args.uid_key)
